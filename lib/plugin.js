module.exports = function(opts) {
  ...
  function processJSXElement(path) {
    ...
    if (hasSpreadAttribute(attributes)) {
      if (renderAsPartial) {
        throw new Error('Custom components with spread are currently not supported');
      }
      markup += processAttributesWithSpread(attributes, tagName);
    } else {
  ...
  function processAttributesWithSpread(attributes, tagName) {
    let objects = [];
    for (let attribute of attributes) {
      if (attribute.isJSXSpreadAttribute()) {
        ...
        if (valuePath.isJSXExpressionContainer()) {
          value = filterThisExpressions(valuePath.get('expression')).node;

...
export default function(opts) {
        ...
        exit(node) {
          if (node.get('source').get('value').node == 'react') {
            ...
            if (specifier.isImportDefaultSpecifier()) {
              ...
              node.insertAfter(
                t.ifStatement(
                  createMemberExpression(t.identifier(name), t.stringLiteral('handlebars-partial')),
                  t.blockStatement([
      ...
      FunctionExpression: {
        ...
        exit(node) {
          if (node.get('id').node !== null && node.get('id').get('name').node == 'render') {
            ...
            node.findParent(path => path.isVariableDeclaration()).remove();
          }
        }
      },
      ClassDeclaration: {
  ...
  function processRenderFunction(path, node) {
    ...
    node.body = Array.prototype.concat(
      [
        ...
        t.ifStatement(
          t.unaryExpression(
            '!',
            createMemberExpression('execOptions', 'data', t.stringLiteral('partial-block'))
          ),
          t.blockStatement([
            t.throwStatement(
              t.newExpression(
                t.identifier('Error'),
                [
                  t.stringLiteral('Calling partials is only valid as block-partial')
                ]
              )
            )
          ])
    ...
    programPath.node.body.push(
      ...
      t.expressionStatement(
        t.assignmentExpression(
          '=',
          createMemberExpression(t.identifier(classID), t.stringLiteral('handlebars-partial')),
  ...
  function processJSXExpressionContainer(path) {
    ...
    if (expression.isCallExpression()) {
            ...
            for (let param of params) {
              ...
              if (varRef) {
                objects.push(
                  t.property(
                    null,
                    t.identifier(varRef.name),
                    t.identifier(name)
                  )
                );
              }
            }
            let body = path.get('body').get('body');
      ...
      let callResultRef = findProgramPath(path).scope.generateUidIdentifier('callResult');
  ...
  function createMemberExpression(...args) {
    ...
    for (let i = 1, n = args.length; i < n; i++) {
      ...
      const computed = args[i] && args[i].type ? args[i].type === 'StringLiteral' : false;
